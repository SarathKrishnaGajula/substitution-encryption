...
def main():
    print('Welcome to playfair-cipher here are the options you can perform: ')
    option = int(input('choose the option you want to perform: '))
    if (option == 0):
        bigrams = get_bigrams()
        key = key_input()
        print(encryption(bigrams, key))
    elif (option == 1):
        digrams = ciphertext_input()
        key = key_input()
        print(decrypt(digrams,key))
